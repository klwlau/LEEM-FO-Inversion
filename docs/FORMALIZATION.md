...
- Vectors are bold ($`\mathbf{q}`$); scalars (including 1D signed spatial frequency) are italic ($`q`$). Magnitudes: $`q=\lvert{\mathbf{q}}\rvert`$.
...
These are identical after $`\alpha=\lambda q`$. **Lean should use the spatial-frequency form**, with $`\lambda`$ explicit, $`q=\lvert{\mathbf{q}}\rvert`$, and $`W:=\exp(2\pi\mathrm{i}\chi)`$ as a definition.

### 0.3 One-dimensional reduction

For a 1D object $`\psi_0(x)`$ the paper replaces 2D vectors by **signed scalars** $`q,q',k\in\mathbb{R}`$. Then $`\mathbf{q}\cdot\mathbf{q}'\mapsto qq'`$ (product of signed reals, not $`\lvert{q}\rvert\lvert{q'}\rvert`$). Odd powers $`q^3,q^5`$ keep the sign. The 2D theory is **not** the 1D theory with $`q_y\equiv 0`$ unless the source integral is also reduced to 1D; the paper’s 1D envelopes use a 1D source density along the object axis. Both 1D and 2D Gaussian Fourier transforms produce the **same** exponent $`-2\pi^2\sigma^2\lvert{a}\rvert^2`$ (see A1).
...
\qquad
q=\lvert{\mathbf{q}}\rvert.
```

- $`\boldsymbol{\alpha}`$: emission-angle vector from the virtual object (paraxial; $`\lvert{\boldsymbol{\alpha}}\rvert\ll 1`$).
...
=
\begin{cases}
1 & \text{if }\lvert{\mathbf{q}}\rvert\le q_{\mathrm{ap}},\\
0 & \text{if }\lvert{\mathbf{q}}\rvert> q_{\mathrm{ap}},
\end{cases}
\qquad
...
```math
\chi_S(\mathbf{q},\Delta z)
=\frac14 C_3\lambda^3\,\lvert{\mathbf{q}}\rvert^4
+\frac16 C_5\lambda^5\,\lvert{\mathbf{q}}\rvert^6
+\frac12\Delta z\,\lambda\,\lvert{\mathbf{q}}\rvert^2.
```

- **nac LEEM:** $`C_3`$ dominates; $`C_5`$ is dropped.
- **ac LEEM:** $`C_3=0`$; $`C_5`$ is kept.

**1D signed-scalar form:** replace $`\lvert{\mathbf{q}}\rvert^{2n}`$ by $`q^{2n}`$ (even).
...
**Thm.** (chain rule on a radial function $`\chi_S=\chi_S(q)`$, $`q=\lvert{\mathbf{q}}\rvert`$)
...
$`\chi_C`$ is radial: it depends on $`q=\lvert{\mathbf{q}}\rvert`$ only, not on $`\arg\mathbf{q}`$. Consequently $`E_{C,\mathrm{tot}}(\mathbf{q},\mathbf{q}')`$ depends on $`(\lvert{\mathbf{q}}\rvert,\lvert{\mathbf{q}'}\rvert)`$ only. The **dot product** $`\mathbf{q}\cdot\mathbf{q}'`$ appears only in $`E_S`$.
...
\exp\Bigl(-\frac{\lvert{\mathbf{k}}\rvert^2}{2\sigma_{\mathrm{ill}}^2}\Bigr),
\qquad
...
because $`\lvert{\mathbf{k}}\rvert^2=k_x^2+k_y^2`$.
...
```math
s(\mathbf{k})=\frac{\beta}{\pi}\,\mathrm{e}^{-\beta\lvert{\mathbf{k}}\rvert^2},
\qquad
...
```math
E_S(\mathbf{q},\mathbf{q}',\Delta z)
=\exp\bigl(-2\pi^2\sigma_{\mathrm{ill}}^2\,\lvert{\mathbf{a}}\rvert^2\bigr).
```

The 1D theorem is the same with $`\lvert{\mathbf{a}}\rvert^2\to a^2`$. Proof: $`\mathbb{E}[\mathrm{e}^{\mathrm{i}\mathbf{t}\cdot\mathbf{k}}]=\mathrm{e}^{-\frac12\sigma_{\mathrm{ill}}^2\lvert{\mathbf{t}}\rvert^2}`$ at $`\mathbf{t}=2\pi\mathbf{a}`$; equivalently, complete the square in rectangular coordinates using $`s=s(k_x)s(k_y)`$.
...
\bigl\lvert\mathbf{a}(\mathbf{q},\mathbf{q}',\Delta z)\bigr\rvert^2
\Biggr).
}
```
...
**Thm.** (collect powers of $`\varepsilon`$ in §5; use $`q=\lvert{\mathbf{q}}\rvert`$, $`q'=\lvert{\mathbf{q}'}\rvert`$)
...
**Remark (conjugate).** The CTF intensity $`\lvert{\mathrm{FT}^{-1}(\boldsymbol{\Psi}H)}\rvert^2`$ produces the bilinear factor $`E_{C,\mathrm{tot}}(\mathbf{q},0)\,E_{C,\mathrm{tot}}(\mathbf{q}',0)^*`$. Equation (7a) as printed uses the **non-conjugated** product $`E(q)E(q')`$. For ac, $`E_{C,\mathrm{tot}}`$ is complex; the paper states that $`\Gamma_C`$ is complex and that **it suffices to plot** $`\lvert{\Gamma_C}\rvert`$ because $`\mathrm{Im}\,E_{C,\mathrm{tot}}`$ is numerically small inside the aperture (Schramm 2012 parameters, Table 1). Lean should:

1. define $`\Gamma_C`$ exactly as (7a);
2. separately record $`\lvert{\Gamma_C}\rvert`$ as the quantity plotted in Fig. 1;
...
A2 is written for **1D** ac-LEEM ($`C_C=0`$; $`C_{CC},C_{3C}`$ kept). All $`q,q'`$ below are signed scalars. The 2D polar form is identical after $`q^2\mapsto\lvert{\mathbf{q}}\rvert^2`$, $`q^4\mapsto\lvert{\mathbf{q}}\rvert^4`$.
...
Proof: $`\lvert{1-\mathrm{i}y}\rvert=\sqrt{1+y^2}`$, $`\arg(1-\mathrm{i}y)=-\arctan y`$, so
...
**Parity.** $`A(q,q')=A(q',q)=A(\lvert{q}\rvert,\lvert{q'}\rvert)`$. $`\theta(q,q')=-\theta(q',q)`$, and $`\theta`$ changes sign with $`(q^2-{q'}^2)`$.
...
$`E_S`$ is real. From (7b) and $`E_S=\exp(-\kappa\lvert{a}\rvert^2)`$ with
...
```math
\Gamma_S
=\exp\bigl(-\kappa\lvert{\mathbf{u}}\rvert^2-\kappa\lvert{\mathbf{v}}\rvert^2+\kappa\lvert{\mathbf{u}-\mathbf{v}}\rvert^2\bigr)
...
(tempered distributions on $`\mathbb{R}`$; on a large finite window the $`\delta`$ become sinc peaks). Larger $`\varphi`$ populates larger $`\lvert{n}\rvert`$, which is why CTF fails for strongly corrugated graphene: the object samples $`(q,q')`$ away from the axes where $`\Gamma_C\Gamma_S\approx 1`$.
...
| `LeemFO/Inverse/Tikhonov.lean` | Scalar/2×2 Fourier-bin Tikhonov (`tikhonovJ`, `tikhonovXhat2`, `tikhonov2_unique`, `kaczmarzSweep`, `reconstructCost`) |
| `LeemFO/Inverse/LinearInverse.lean` | Linearized slice identifiability (`R_FO_axis_eq_zero_iff`, `ihat_gauge`, `ihatJac_vacuum`, `ihat_hermitian`) |
| `LeemFO/Inverse/Pipeline.lean` | Fourier-domain inverse map (`stage1Scalar`, `stage1Pair`, `vacuumGN_eq_stage1Pair`, `stage2Skip`) |
| `LeemFO/Inverse/LowRank.lean` | Rank-1 / TCC / LR+D apply (`ihat_tcc`, `lrPlusDiag`, `recommendTccRank`, `hybridCost`) |
| `LeemFO/Inverse/Homotopy.lean` | Exact quadratic homotopy, Born remainder, `mixedSpectrumPair` |
| `LeemFO/Inverse/LineSearch.lean` | Exact quartic line energy (`lineFid_quadEnergy`, `descentStep`, `nlsJ_line`, `exists_quadA2_ne_gn`) |
| `LeemFO/Inverse/Plane2.lean` | 2D stage-1 map (`stage1Pair2`, `vacuumGN_eq_stage1Pair2`) |
| `LeemFO/Inverse/Mix.lean` | End-to-end `mixed2DMix`; TIE / HIO / Rytov / Wiener / radial / JA witnesses |

**Linearized Fourier-diagonal inverse (see [LINEAR_INVERSE.md](LINEAR_INVERSE.md)).** Multi-defocus Tikhonov on the CTF slice $`R_{\mathrm{FO}}(q,0,\Delta z)`$ has a unique minimizer for $`\alpha>0`$, the exact bias–noise identity $`\hat x-x^\star=(\sum \overline h n-\alpha x^\star)/D`$, and the sharp triangle bound. The vacuum $`2\times 2`$ Gram has a Cramer closed form (`tikhonovXhat2`) that is the unique minimizer for $`\alpha>0`$. The reconstruction map is `stage1Scalar` / `stage1Pair` in `Pipeline.lean` and `stage1Pair2` on $`R_{\mathrm{FO}2}`$. Modes with $`\lvert{q}\rvert>q_{\mathrm{ap}}`$ are identically invisible. Bilinear FO is phase-gauge invariant; its vacuum linearization has an exact quadratic remainder (one Gauss–Newton step from vacuum *is* `stage1Pair`). Cost is modelled as $`O(KN\log N)`$ DFTs plus $`O(KN)`$ bin solves, without proving FFT existence. $`K=1`$ cannot identify a general complex $`(X(q),\overline{X(-q)})`$ pair; weak-phase CTF zeros of $`\sin(2\pi\chi_S)`$ sit inside a large enough aperture. Stage 2 is skipped when the bilinear remainder is at a noise floor $`\eta`$ (`stage2Skip`); the numerical $`\max\lvert{\varphi}\rvert\lesssim 0.3`$ remains informal. Statistical noise models remain informal.

**Large-phase 2D mix.** For $`\varphi\not\ll 1`$ the recommended 2D estimator is that stage-1 init, then remainder-corrected Born steps whose apply is rank-$`M`$ TCC (`ihat_tcc`, `recommendTccRank`) or coherent rank-1 autocorrelation, with optional (not cheaper) LR+D (`lrPlusDiag`), per-bin skip (`remainderWeight` / `mixed2DMix`) and algebraic Armijo on the exact quartic (`lineDescentStep`). Gauss–Newton’s Hessian is not the exact quartic Hessian (`exists_quadA2_ne_gn`). The mix is FO-faithful because bilinear FO is exactly quadratic (`ihat_homotopy_cubic_zero`). A rank-$`M\le 8`$ Born step is cheaper than a dense pair apply at $`N=128`$; coherent rank-1 line search is also cheaper at that grid, rank-$`8`$ line search is not. Hermitian data and odd $`qmap`$ (not odd $`\lvert G\rvert`$) recover $`\delta(-\xi)=\overline{v(\xi)}`$ (`mixed2D_conj_partner`). Do not fold Jacobi–Anger into `mixed2D`; do not pin $`\operatorname{Im}(\delta 0)`$ for uniqueness. The 1D mix stays fidelity-only (`nlsJ_line`); do not put $`\alpha`$ into `lineFid`. TIE (also on $`R_{\mathrm{FO}2}`$), coherent GS / HIO / ER / $`R_{\mathrm{CTF}}`$, first-order Rytov increment, single-defocus Wiener, a radial 1D kernel, Jacobi–Anger as a general 2D inverse, and forgetful Kaczmarz fail as identities (`Mix.lean`, `Tikhonov.lean`).

**Inverse (journal-style note: [proofs/leemfo_inverse.pdf](proofs/leemfo_inverse.pdf); algorithm catalogue: [proofs/leemfo_mix.pdf](proofs/leemfo_mix.pdf)).** Lean details stay in [LINEAR_INVERSE.md](LINEAR_INVERSE.md). Scientific verdict: for a 2D experimental through-focal stack the linearized map that still fills CTF zeros is Fourier-diagonal multi-defocus Tikhonov on the slice $`R_{\mathrm{FO}}(q,0,\Delta z)`$ (Schiske/Wiener), cost $`O(KN\log N)`$. That map is the Fréchet derivative of bilinear FO at vacuum, the mix init, and the quiet-bin branch; it is biased for $`\varphi=4\pi A/\lambda_0\not\ll 1`$. Stage 2 for large phase is the mixed Born homotopy with rank-$`M`$ TCC apply (exact rank-1 under perfect coherence), per-bin $`t\in\{0,1\}`$ skip, and optional quartic Armijo, not a dense $`KN^2`$ Gauss--Newton loop. For the paper's 1D sinusoid, stage 2 collapses to Jacobi--Anger least squares on $`\{J_n(\varphi)\}_{\lvert{n}\rvert\le\lfloor q_{\mathrm{ap}}\Lambda\rfloor}`$, cost $`O(M^2)`$ with $`M=2\lfloor q_{\mathrm{ap}}\Lambda\rfloor+1`$. Gerchberg--Saxton is either misspecified under partial coherence ($`\Gamma_S\neq 1`$ off axis) or a slower rewrite of Gauss--Newton. MAL/phase diversity is Gauss--Newton stacked over extra defoci, needed only when aberrations are unknown. Uniqueness of the regularized Fourier-diagonal estimator, the quadratic homotopy, the TCC apply, and the quartic line energy are machine-checked in Lean 4.
