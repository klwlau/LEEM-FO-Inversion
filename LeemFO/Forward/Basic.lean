...
/-- Spherical (defocus-independent) radial prefactor of `∇χ_S`. -/
noncomputable def uS0 (q : EuclideanSpace ℝ (Fin 2)) : ℝ :=
  p.C3 * p.lam ^ 3 * ‖q‖ ^ 2 + p.C5 * p.lam ^ 5 * ‖q‖ ^ 4

/-- Radial prefactor of `∇χ_S`: `∇χ_S(q) = u_S(q) • q`. -/
noncomputable def uS (q : EuclideanSpace ℝ (Fin 2)) (Δz : ℝ) : ℝ :=
  p.uS0 q + Δz * p.lam

lemma uS_eq (q : EuclideanSpace ℝ (Fin 2)) (Δz : ℝ) :
    p.uS q Δz
...
lemma aS2_pureDefocus (hC3 : p.C3 = 0) (hC5 : p.C5 = 0)
    (q q' : EuclideanSpace ℝ (Fin 2)) (Δz : ℝ) :
    p.aS2 q q' Δz = (Δz * p.lam) • (q - q') := by
  simp [aS2, uS, uS0, hC3, hC5, smul_sub]
...
/-- Spherical part of `χ_S` (no defocus). -/
noncomputable def chiS2sph (q : EuclideanSpace ℝ (Fin 2)) : ℝ :=
  (1 / 4 : ℝ) * p.C3 * p.lam ^ 3 * ‖q‖ ^ 4
    + (1 / 6 : ℝ) * p.C5 * p.lam ^ 5 * ‖q‖ ^ 6

lemma chiS2_split (q : EuclideanSpace ℝ (Fin 2)) (Δz : ℝ) :
    p.chiS2 q Δz = p.chiS2sph q + (1 / 2 : ℝ) * Δz * p.lam * ‖q‖ ^ 2 := by
  simp [chiS2, chiS2sph]

lemma chiS2_pureDefocus (hC3 : p.C3 = 0) (hC5 : p.C5 = 0)
    (q : EuclideanSpace ℝ (Fin 2)) (Δz : ℝ) :
    p.chiS2 q Δz = (1 / 2 : ℝ) * Δz * p.lam * ‖q‖ ^ 2 := by
  simp [chiS2, hC3, hC5]
