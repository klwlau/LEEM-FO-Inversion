...
Rank-adaptive `M` is chosen by `recommendTccRank` from the
`ihat_tcc_trunc_bound` proxy versus the cost-safe caps `M=1` (perfect
coherence / line search) and `M≤8` (Born/hybrid at `N=128`);
zero-weight padding is free (`tccKernel_insert_weight_zero`).
-/
...
/-- LR+D with the Born/hybrid rank cap stays cheaper than dense at `N = 128`. -/
theorem lrPlusDiagApplyCost_recommend_lt_dense {K : ℕ} (hK : 1 ≤ K)
    (truncOk : ℕ → Bool) :
    lrPlusDiagApplyCost K (recommendTccRank false false truncOk) 128
      < denseApplyCost K 128 := by
  have hM : recommendTccRank false false truncOk ≤ 8 := by
    simpa [recommendTccCap_born] using
      recommendTccRank_le_cap false false truncOk
  exact lrPlusDiagApplyCost_lt_dense_128 hK hM
